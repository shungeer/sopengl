...
void BuildShader()
{
	...
	gShaders[2] = new CShader("shaders/color-v.vs", "shaders/color-v.fs");
	gShaders[3] = new CShader("shaders/color-s.vs", "shaders/color-s.fs");
}
// 创建所有渲染模型
void BuildModel()
{
...
void DrawContext2()
{
	...
	gShaders[2]->setVecf3("objectColor", 1.0f, 0.5f, 0.31f);
	gShaders[2]->setVecf3("lightColor", 1.0f, 1.0f, 1.0f);
...
void DrawContext3()
{
	...
	gShaders[3]->setVecf3("lightPos", lightPos.x(), lightPos.y(), lightPos.z());
	gShaders[3]->setVecf3("objectColor", 1.0f, 0.5f, 0.31f);
	gShaders[3]->setVecf3("lightColor", 1.0f, 1.0f, 1.0f);
	gShaders[3]->setMat4("projection", projection);
	gShaders[3]->setMat4("view", view);
	gShaders[3]->setMat4("model", model);
	gShaders[3]->setVecf3("material.ambient", 1.0f, 0.5f, 0.31f);
	gShaders[3]->setVecf3("material.diffuse", 1.0f, 0.5f, 0.31f);
	gShaders[3]->setVecf3("material.specular", 0.5f, 0.5f, 0.5f);
	gShaders[3]->setFloat("material.shininess", 32.0f);
	
	Vec3<float> lightColor;
	lightColor[0] = sin((float)glfwGetTime() * 2.0f);
	lightColor[1] = sin((float)glfwGetTime() * 0.7f);
	lightColor[2] = sin((float)glfwGetTime() * 1.3f);

	Vec3<float> diffuseColor = lightColor   * Vec3<float>(0.5f, 0.5f, 0.5f); // 降低影响
	Vec3<float> ambientColor = diffuseColor * Vec3<float>(0.2f, 0.2f, 0.2f); // 很低的影响
	gShaders[3]->setVecf3("light.ambient", ambientColor.x(), ambientColor.y(), ambientColor.z());
	gShaders[3]->setVecf3("light.diffuse", diffuseColor.x(), diffuseColor.y(), diffuseColor.z()); // 将光照调暗了一些以搭配场景
