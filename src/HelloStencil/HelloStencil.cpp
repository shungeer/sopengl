...
int main(int argc, CHAR* argv[])
{
	...
	glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);	// 模板更新动作
...
void DrawContext1()
{
	...
	// 1.绘制模板
	glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);	// 控制颜色缓冲之中的所有通道都不被绘制
	glDepthMask(GL_FALSE);	// 之后每一帧绘制物体的深度都不写入深度缓存
	glStencilMask(0xFF);
	...
	// 2.绘制实景，不写入模板缓存
	glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);	// 绘制所有颜色缓冲之中的通道分量
	glDepthMask(GL_TRUE);	// 完成深度测试，并写入深度缓存中
	glStencilMask(0x00);
...
void DrawContext2()
{
	...
	// 2.绘制立方体，先绘制完整立方体（通过模板测试，并将模板写入模板缓存中）
	glStencilFunc(GL_ALWAYS, 1, 0xFF);
	glStencilMask(0xFF);
	// cube1
	glBindVertexArray(gVAOs[0]);
	...
	// 3.绘制稍微大一点的立方体，这次禁止模板缓存的写入
	// 上述立方体部分模板都为1，所以接下来绘制模板不等于1的部分
	glStencilFunc(GL_NOTEQUAL, 1, 0XFF);
	glStencilMask(0x00);
	glDepthFunc(GL_ALWAYS);
	//glDisable(GL_DEPTH_TEST);
	//glDepthMask(GL_FALSE);	// 深度缓存接下来每一帧都不会写入绘制物体的深度
	float scale = 1.1f;
