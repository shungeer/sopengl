...
namespace cell
{
	...
	public:
		...
		Mat4<value_type>& translate(value_type x, value_type y, value_type z)	// 平移
		{
			...
			m_aM[13] += m_aM[1] * x + m_aM[5] * y + m_aM[9] * z;
			m_aM[14] += m_aM[2] * x + m_aM[6] * y + m_aM[10] * z;
			m_aM[15] += m_aM[3] * x + m_aM[7] * y + m_aM[11] * z;
		...
		Mat4<value_type>& scale(value_type x, value_type y, value_type z)
		{
			...
			m_aM[0] *= x;   m_aM[4] *= y;   m_aM[8]  *= z; 
			m_aM[1] *= x;   m_aM[5] *= y;   m_aM[9]  *= z;   
			m_aM[2] *= x;   m_aM[6] *= y;   m_aM[10] *= z;   
			m_aM[3] *= x;   m_aM[7] *= y;   m_aM[11] *= z;   
			return *this;
		}
		...
		Mat4<value_type>& rotate(value_type angle, value_type x, value_type y, value_type z)
		{
			...
			value_type m0 = m_aM[0], m4 = m_aM[4], m8 = m_aM[8], m12 = m_aM[12],
				...
				m3 = m_aM[3], m7 = m_aM[7], m11 = m_aM[11], m15 = m_aM[15];
			...
			m_aM[0] = r0 * m0 + r1 * m4 + r2 * m8;
			m_aM[1] = r0 * m1 + r1 * m5 + r2 * m9;
			m_aM[2] = r0 * m2 + r1 * m6 + r2* m10;
			m_aM[3] = r0 * m3 + r1 * m7 + r2* m11;
			m_aM[4] = r4 * m0 + r5 * m4 + r6 * m8;
			m_aM[5] = r4 * m1 + r5 * m5 + r6 * m9;
			m_aM[6] = r4 * m2 + r5 * m6 + r6 * m10;
			m_aM[7] = r4 * m3 + r5 * m7 + r6 * m11;
			m_aM[8] = r8 * m0 + r9 * m4 + r10 * m8;
			m_aM[9] = r8 * m1 + r9 * m5 + r10 * m9;
			m_aM[10] = r8 * m2 + r9 * m6 + r10* m10;
			m_aM[11] = r8 * m3 + r9 * m7 + r10* m11;
			return *this;
		}
		Mat4<value_type>& rotateX(value_type angle)
		{
			value_type c = cos(cell::DegreesToRadians(angle));    // cosine
			value_type s = sin(cell::DegreesToRadians(angle));    // sine
			value_type m1 = m_aM[1], m2 = m_aM[2], m4 = m_aM[4],
				m5 = m_aM[5], m6 = m_aM[6], m7 = m_aM[7],
				m8 = m_aM[8], m9 = m_aM[9], m10 = m_aM[10],
				m11 = m_aM[11], m13 = m_aM[13], m14 = m_aM[14];
		...
		Mat4<value_type>& rotateY(value_type angle)
		{
			value_type c = cos(cell::DegreesToRadians(angle));    // cosine
			value_type s = sin(cell::DegreesToRadians(angle));    // sine
			value_type m0 = m_aM[0], m1 = m_aM[1], m2 = m_aM[2], m3 = m_aM[3],
				m4 = m_aM[4], m6 = m_aM[6],
				m8 = m_aM[8], m9 = m_aM[9], m10 = m_aM[10], m11 = m_aM[11],
		...
		Mat4<value_type>& rotateZ(value_type angle)
		{
			...
			value_type s = sin(cell::DegreesToRadians(angle));    // sine
			value_type m0 = m_aM[0], m1 = m_aM[1], m2 = m_aM[2], m3 = m_aM[3],
				m4 = m_aM[4], m5 = m_aM[5], m6 = m_aM[6], m7 = m_aM[7],
				m8 = m_aM[8], m9 = m_aM[9], m10 = m_aM[10], m11 = m_aM[11],
