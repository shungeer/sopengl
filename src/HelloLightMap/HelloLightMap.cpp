...
void BuildShader()
{
	...
	gShaders[2] = new CShader("shaders/color-inv.vs", "shaders/color-inv.fs");
	gShaders[3] = new CShader("shaders/color-emission.vs", "shaders/color-emission.fs");
}
// 创建所有渲染模型
void BuildModel()
{
...
void BuildTexture()
{
	...
	LoadTexImage("../../context/resource/matrix.jpg", gTexs[2]);
}
// 为着色器纹理变量指定纹理单元
void InitTexture()
{
	gShaders[0]->use();
	gShaders[0]->setInt("material.diffuse", 0);
	gShaders[0]->setInt("material.specular", 1);
	...
	gShaders[3]->setInt("material.emission", 2);
}
// 清除渲染上下文
void ClearContext()
{
...
void DrawContext1()
{
	...
	gShaders[0]->setFloat("material.shininess", 64);
	gShaders[0]->setVecf3("light.ambient", 0.2f, 0.2f, 0.2f);
	gShaders[0]->setVecf3("light.diffuse", 0.5f, 0.50f, 0.5f); // 将光照调暗了一些以搭配场景
